...
app.controller('PickupDateResultsCtrl', [ '$scope', '$timeout', '$http','$state', 'toaster', '$localStorage','$stateParams',
		function($scope, $timeout, $http, $state, toaster, $localStorage, $stateParams) {
			...
			function coverttoexportdataformat(dataCollection) {
				var dataarray = [];
				var keyarray = [ "Probill", "Pickup Date", "Shipper", "City", "Consignee","City", 
				                 "Delivery Date","Delivery Time", "BOL #","PO #","Shipper #",
				                 "Weight","Adj. Wgt","Received By","Terms"];
			...
			$scope.exportcsv = function(exportOption) {
				...
				$scope.csvfilename = 'ProBills_by_Pickup_Date_'+$scope.beginDate+'-'+$scope.endDate+'.csv';
			...
			$scope.exportpdf = function(exportOption) {
				...
				pdfMake.createPdf(docDefinition1).download('ProBills_by_Pickup_Date_'+$scope.beginDate+'-'+$scope.endDate+'.pdf');
			}
			...
			$scope.seeDetails = function(probillNo) {
				...
				$http({
					...
					$localStorage.original = 'app.tracing.pickupdateresults';
