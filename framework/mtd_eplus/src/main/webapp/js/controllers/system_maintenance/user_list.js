app.controller('userlistctrl', ['$scope', '$timeout', '$http', '$state', 'toaster', '$localStorage',
    function ($scope, $timeout, $http, $state, toaster, $localStorage) {
        ...
        $scope.getData = function (searchdata) {
            ...
            $http({
                method: 'GET',
                url: "maintenance/userManager/retrieveSecureUsers.do",
                params: {
        ...
        $scope.removeUsers = function (id) {
            if (confirm('Are you sure you want to delete this record?')) {
                ...
                $http({
                    method: 'GET',
                    url: "maintenance/userManager/removeUsers.do",
                    params: {
                        "euIds": removeItems
                    }
                }).then(
            ...
            angular
                .forEach(
                    ...
                    function (item, key) {
                        ...
                        valuearray.push(valutils.trimToEmpty(dataCollection[key].customer) + "");
                        valuearray.push(valutils.trimToEmpty(dataCollection[key].uid) + "");
        ...
        $scope.exportcsv = function (exportOption) {
            ...
            $scope.csvfilename = 'User_Management.csv';
        ...
        $scope.exportpdf = function (exportOption) {
            ...
            pdfMake.createPdf(docDefinition1).download('User_Management.pdf');
        }

        $scope.editUser = function (euId) {
            toaster.pop('wait', '', 'Loading...');
            var param = {
                euId: euId,
                status: "from_edit"
            };
            $state.go('app.system_maintenance.user_edit', param);
