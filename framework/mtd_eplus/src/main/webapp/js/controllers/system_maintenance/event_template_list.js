app.controller('templatelistctrl', ['$scope', '$timeout', '$http', '$state', 'toaster', '$localStorage',
    function ($scope, $timeout, $http, $state, toaster, $localStorage) {
        ...
        var getData = function () {
            ...
            $http({
                method: 'GET',
                url: "maintenance/tripEventTemplate/retrieveTripEventTemplateListByType.do",
                params: {
        ...
        $scope.exportcsv = function (exportOption) {
            ...
            $scope.csvfilename = 'Event_Template_Management.csv';
        ...
        $scope.exportpdf = function (exportOption) {
            ...
            pdfMake.createPdf(docDefinition1).download('Event_Template_Management.pdf');
        }
        ...
        $scope.submitTemplate = function(isValid) {
            if (isValid && $scope.templateEntity)
            {
                $http({
                    method: 'POST',
                    url: "maintenance/tripEventTemplate/saveEventName.do",
                    params: $scope.templateEntity
                }).then(function successCallback(response) {
                    if (response.data.result == 'duplicate') {
                        toaster.pop('error', '', "Event Name CANNOT be duplicated, please choose a new one!");
                        return;
        ...
        $scope.editTemplate = function (type, name) {
            ...
            $state.go('app.system_maintenance.event_templates', param);
        }
        ...
        $scope.removeTemplate = function (type, name) {
            ...
            $http({
                method: 'GET',
                url: "maintenance/tripEventTemplate/removeEventTemplateByCategory.do",
                params: {
                    "tripType": type,
                    "name": name
                }
